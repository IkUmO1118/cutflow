# モーション(CSS/WAAPI 既定 + GSAP/Lottie は pin 経由)
...
経由で seek** し、さらに **GSAP 3.14.2 と Lottie 5.12.2 を pin 済み**
(`src/lib/hyperframeCdn.ts`)で、GSAP は `window.__timelines["<composition-id>"]`
へ登録した **paused timeline**、Lottie は `window.__hfLottie` 登録の
animation を、いずれも絶対時刻へ seek して駆動する。したがって **GSAP 前提の
記法は使える**(pin タグ逐語+`data-hf-requires="gsap"` が条件)。Three.js/
Anime.js/TypeGPU は未 pin で対象外。本書は CSS/WAAPI アダプタ(最も単純で
byte 決定的な既定経路)と `hyperframes-keyframes` の seek-safe な作法を
Cutflow 向けに書き直す。GSAP を pin して使うときの tween の作法は下記「GSAP を pin して使うとき」節、Lottie は authoring-contract の B4 と下記「Lottie アダプタ」節にまとめる。
