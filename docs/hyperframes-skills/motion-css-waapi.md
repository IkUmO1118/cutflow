# モーション(CSS/WAAPI 既定 + pin runtime + raw WebGPU)
...
Three.js は`hf-seek`の絶対秒で同期描画し、raw WebGPUは`webgpu` capability tokenと
readiness PromiseでWGSL pipelineを駆動する。したがって **GSAP/Anime.js/Three.js/
...
X4のnative/manual routeの要点(5点。TypeGPU runtimeは引き続きOUT):

1. WebGLでは不足するWGSL/WebGPU-native表現にだけ使い、rootへ
   `data-hf-requires="webgpu"`と`data-hf-determinism="perceptual"`を宣言する
2. 最初の`await`前に同期`hf-seek` listenerを置き、絶対秒を`latestTime`へ保持する。
   初期化中のseekを捨てず、GPU準備後も最新時刻を1回描く
3. adapter/device/context/shader/pipelineの非同期初期化は
   `window.__hyperframes.__ready`のPromiseへ閉じる。`device.lost`はfatal channelへ送り、
   `getCompilationInfo().messages`のerrorをpipeline作成前にthrowする
4. frameごとに時刻uniformを絶対値で上書きし、新しいcommand encoderから
