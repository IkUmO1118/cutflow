...
FrameWright の native な作図契約(C1/C2)に意味があるものだけを翻案した記録。
...
| `hyperframes-animation` | **SELECT(css/waapi adapter のみ精読)** | GSAP 前提の6 adapter は不採用。CSS/WAAPI の2 adapter だけが FrameWright の native interpreter と一致する |
...
| `faceless-explainer` | **SELECT(デザイン感性のみ採取)** | 実写素材が無い前提でタイポグラフィ/図解/データ可視化だけを発明する原則が、FrameWright のカード生成(素材が無い状態から HTML を書く)と完全に一致。CLI/audio/subagent 機構は不採用 |
...
| `pr-to-video` | **SELECT(デザイン感性のみ採取)** | 「navy code surface」のコードカード発想を `card-patterns.md` #5 に単純化して採取。`gh` 連携・diff 抽出・credits close 等は不採用(そもそも FrameWright のカードに PR 入力は無い) |
...
| `talking-head-recut` | REJECT | 実写トーキングヘッド映像の再編集が前提。FrameWright のカードは無音・実写無しの素材専用 |
| `music-to-video` | REJECT | 音楽駆動のビート同期編集。FrameWright のカードは無音(BGM は `bgm.json` の別レイヤーで扱う) |
| `figma` | REJECT | Figma MCP 連携によるアセット取込み。FrameWright はローカルファースト+オフラインが前提で外部 API 連携を持ち込まない |
| `remotion-to-hyperframes` | REJECT | Remotion → HyperFrames への移植ツール。FrameWright は逆方向(HyperFrames の契約を Remotion の native interpreter で実装する側)なので不要 |
| `hyperframes-cli` | REJECT | `npx hyperframes init/lint/check/snapshot/preview/render` 等の CLI 操作。FrameWright は自前の `validate`/`apply`/`frames`/`render` に契約を実装するので CLI 自体は持ち込まない |
| `hyperframes-registry` | REJECT | `npx hyperframes add <block>` のコンポーネントレジストリ。ネットワーク経由の block 取得は FrameWright のオフライン方針(check ゲートのリモート URL 禁止)と相容れない |
| `media-use` | REJECT | HeyGen カタログ/ブランドロゴのオンライン解決。FrameWright の素材は収録フォルダの `materials/` がローカルの正 |
| `embedded-captions` | REJECT | HyperFrames 側の字幕焼き込み機構。FrameWright は独自のテロップ契約(`transcript.json` の `CaptionStyle`)を既に持つ |
| `general-video` | REJECT | 長尺・ナレーション有り・複数シーンの汎用動画制作フロー全体(director/builder/frame-worker のフルオーケストレーション)。FrameWright のカードは単一・無音・自己完結の素材に限定 |
| `product-launch-video` | REJECT | プロダクトサイトのキャプチャからのプロモ動画。FrameWright のカードにサイトキャプチャは無い |
| `slideshow` | REJECT | 画像スライドショー生成。FrameWright は既存の `materials`/`overlays` で同等の役割を持つ |
| `hyperframes`(メイン router) | REJECT | 上記スキル群への意図分岐ルータそのもの。FrameWright 側は `prompts/hyperframe.md` 1本+固定パターンメニューに置き換えたので不要 |
...
- [`authoring-contract.md`](./authoring-contract.md) — `hyperframes-core`
  の契約を FrameWright 向けに要約(root/clip/variables/determinism)
