...
現状は CSS / WAAPI / SVG / DOM / Canvas 2D / GSAP が `usable`、Lottie JSON は
AE 素材持ち込み時だけの `material-routed`。生 WebGL/shader と three.js は
規約・check は整備済みだが GL render profile が未配線の `not-wired`。
