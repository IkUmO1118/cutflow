...
システム音声(systemAudio)・プレビュー幅・エンコーダ・元収録ファイルが
前回の生成と同じなら陳腐化なしと判定する。無ければ常に「陳腐化なし」
扱いになる) / `render.chunks/`(チャンク差分レンダーのキャッシュ。
...
## 出力プロファイル(内部実装・v1 では未使用)

将来のショート動画対応(縦1080x1920)に向けて、出力サイズとベース映像の
レイアウトを「プロファイル」としてまとめる土台がある(`src/lib/profile.ts`)。
組み込みプリセットは `default`(横・現行のワイプ経路)/ `vertical`(縦・
camera上+screen下スタック)/ `vertical-cover`(縦・camera全画面)の3種。
**config.yaml には追加しない**(プリセットは閉じた組み込み定数。設定爆発の
回避)。現時点ではどのコマンドも `default` 固定で呼び出しており、観測できる
挙動の変化はない。ショート書き出し機能(`shorts.json` 経由でクリップごとに
プロファイルを選ぶ)が入るまでは内部実装にとどまる。
