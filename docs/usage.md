...
決定論 tier、CDN pin の URL/version、authoring 経路、`usable` 経路を実測する
render fixture を同時に確認できる。自動処理では固定 `schemaVersion` を持つ
純 JSON を使う:

    node src/cli.ts hyperframe-backends
    node src/cli.ts hyperframe-backends --json

現状は CSS / WAAPI / SVG / DOM / Canvas 2D / GSAP / 生 WebGL/shader が
`usable`、Lottie JSON は AE 素材持ち込み時だけの `material-routed`。
three.js は GPU profile は利用可能だが CDN pin が無いため `not-wired`。
anime.js / D3 / TypeGPU / maps 系 / `.lottie` container は `out`。
...
Lottie(人間持ち込み AE 素材。LLM は作図しない)の JSON は `animationData`
としてカードにインライン埋め込みする(`path:` fetch は禁止=CSP
`connect-src 'none'` でブロックされ、cache key(html sha256)にも乗らない)。
`autoplay:false` / `loop:false` / `window.__hfLottie.push(anim)` は必須。
画像 asset は `assets[].p` を `data:` URL にし、外部 directory(`u:`)・画像
ファイル参照は使わない。対応は lottie-web JSON のみで `.lottie` コンテナは
未対応。
SVG レンダラ(既定)は byte、`renderer:'canvas'` は
...
依存なので byte 一致を一般化せず perceptual tier で検証する。three.js は
未ピン留め(生 WebGL が現状のサポート対象 GPU 作図経路。詳細は
