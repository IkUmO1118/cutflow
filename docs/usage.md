...
node src/cli.ts hyperframe-check <dir> --name <name> --no-vlm      # 現状ノーオペ(VLM lane 未実装)
```

出力は `hyperframe.probe/<name>/index.json`(カードの sha256・寸法・
determinism tier・グリッド情報・findings)。still 撮影 + VLM 二次確認は
まだ実装されていない(スタブ。`stills: []` / `vlm.ran: false` を書く)。
