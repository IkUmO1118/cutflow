...
- `text`: 自由文生成
- `structured`: plan / remeta / plan-shorts / plan-materials / plan-effects / editor AI 提案の schema 付き出力
...
- plan / remeta / plan-shorts / plan-materials / plan-effects / editor AI 提案: provider failure はその操作の error。
  editable JSON は変えない。
...
`plan-materials` / `plan-effects` / `run` / `ingest` / `transcribe` / `detect` / `preview` / `thumbnail` /
...
`remeta` / `plan-shorts` / `plan-materials` / `plan-effects` の LLM プロンプトに自動で注入される。
