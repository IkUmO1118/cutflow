...
- **zooms**: 画面の一部を拡大して見せる(Ken Burns 的な寄り)。「画面のこの
  ...
  - **連鎖(パン遷移)**: 隣のズームの gap(前の `end` 〜 次の `start`)が
    ...
    **gap が 0 より大きい連鎖の gap 区間自体(前 rect の保持)は現状未対応**
    (母艦後段の D2b で対応予定。gap=0 の完全隣接は従来どおり継ぎ目なし)
...
- **書き込み前検査(all-or-nothing)**: 組んだ演出下書きを、書く前に
  ...
  1つでも不正なら1バイトも書かない
- **初版測定はwrite-once**: validate成功後の決定論変換済み演出と、元秒へ戻せる
  上限付きnone判断を`plan-effects.first.json`へ初回だけ保存する。既存ファイルは
  壊れていても`--force`で上書きしない。`describe --json`は現在の演出分類coverageと、
  初版からの型遷移・削除(`to:none`)・noneからの採用(`from:none`)・追加を返す。
  rectの微調整は型反転に数えない
