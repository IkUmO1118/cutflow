...
- `start` / `end` は他の編集ファイルと同じく**元収録の秒**。ツールがカット後の
  ...
  **挿入クリップ(`overlays.json` の `inserts[]`)をまたぐ区間も繋がる**
  (挿入の間だけ BGM が消えることはない)
- `timebase` で番地の軸を選べる(省略時 `"source"`)。**`"output"` にすると
  `start` / `end` を「出力(カット後・挿入込み)の秒」として直接使う**。
  元収録に対応する時刻が無い場所 — 冒頭 intro・末尾 ending・挿入クリップの
  中 — へ BGM を当てられるのはこの軸だけ(下記「挿入クリップと BGM」参照)
...
intro / ending / 途中差し込みの映像(`overlays.json` の `inserts[]`)は、
**元収録に対応する時刻を持たない**。元収録の秒で書かれた `bgm.json` の区間は
本来そこを指せないが、`timebase` を省略した(= `"source"` の)トラックは
**挿入をまたいでひと続きに繋がる**ので、既定では「intro の間だけ BGM が
途切れる」ことは起きない。

```
keeps [0,30] / 挿入 { at: 8, durationSec: 4 } → 挿入は出力 8–12 秒

{ "start": 5, "end": 16, "file": "bgm.mp3" }        ← timebase 省略
  → 出力 5–20 秒を1本で流す(出力 8–12 の挿入の上でも鳴り続ける)
```

**逆に「挿入の間だけ BGM を止めたい / 音量を変えたい」ときは**、
挿入クリップが自前の音楽やナレーションを持っていて重ねると濁る場合など、
`timebase: "output"` のトラックへ書き換えて挿入の前後に分ける。

```jsonc
{
  "tracks": [
    { "timebase": "output", "start": 5,  "end": 8,  "file": "bgm.mp3" },
    // 挿入(出力 8–12)は覆わない → その間 BGM は無音
    // startFrom で曲の続きから(前半で 3 秒鳴っているので 3)
    { "timebase": "output", "start": 12, "end": 20, "file": "bgm.mp3", "startFrom": 3 }
  ]
}
```

出力秒は自分で数えなくてよい。`describe <dir> --json` の
`overlays.inserts[].out`(`{start, end}`)が挿入クリップの出力秒そのもの。

**「全部のトラックで挿入をまたぐのをやめる」設定は用意していない。**
またぎたいトラックとまたぎたくないトラックは同じ動画の中に混在するので、
プロジェクト全体の on/off では粒度が粗すぎる。上のようにトラック単位で
`timebase` を選ぶのが正しい表現方法。

> GUI エディタでは `timebase:"output"` のトラックは**読み取り専用**として
> 表示される(エディタの時刻編集はすべて元収録の秒を前提にしているため)。
> 編集は `bgm.json` を直接、または `apply` で行う。
