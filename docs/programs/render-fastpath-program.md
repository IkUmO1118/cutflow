...
### P2: ffmpeg 区間レンダラ(video only) — 状態: 未着手
...
### P3: ハイブリッド組み立てと render 結線(opt-in) — 状態: 未着手
...
- **2026-07-13 P1 完了**(Opus 設計→Sonnet 実装→コーディネータ実測検証)。
  ...
  PNG をベース映像 span へ overlay する ffmpeg 区間レンダラを作る。**次は P2**。

---

## 9. 次セッションの着手手順(P0・P1 は完了済み → **次は P2**)
...
- 色変換フィルタ:
  `scale=in_range=limited:out_range=full,colorspace=all=smpte170m:iall=bt709:range=pc,format=yuvj420p`
- 区間切り出しは `trim=start_frame=N:end_frame=M,setpts=PTS-STARTPTS`(秒指定禁止)
