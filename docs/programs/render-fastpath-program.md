...
### P6: 計測・既定化判断 — 状態: **完了(2026-07-14。数値の正は docs/perf.md フェーズ11)**

- perf.md へ実測追記(cold 時間・PSNR・リソース。フェーズ9のモニタ手法を再利用)
- 数収録の実運用で問題が出なければ `render.fastPath: true` を既定化

**実施結果(2026-07-14。実収録 2026-07-12・BGM×3 そのまま・cut.mp4 温存 cold)**:

- **cold render: full 202.6s(Remotion 段 188.5s)→ fast 102.2s / 92.5s(2回)
  = 約50〜55%短縮・約2.1倍速**。`FAST 1 / SLOW 0(被覆 100.0%, 音声 bgm-mix)`
- **リソース(フェーズ9の1秒サンプリング手法。未測定だった最後の項目)**:
  併走 RSS ピーク **6.7GB → 2.7GB**、chrome-headless **59 → 5 プロセス**、
  システム空きメモリ最小 **23MB → 1.8GB**、スワップ増はどちらも 0。
  ただし FAST の ffmpeg は PNG 入力数に比例して食う(107 入力で単体 2.4GB)
- **等価性**: 両者 6301f / 210.033333s、全編 PSNR avg 42.52dB / min 33.04dB /
  `<30dB` 0 frame(P5-1 と同値)、統合ラウドネス両方 -14.2 LUFS(差 0.0 LU)
- **副産物: フル(Remotion)経路の非決定 +1 frame フレークを観測** — 本日の
  フルレンダー2回中1回が 6302f(frame 3567 付近に1枚重複。+1 シフトで以降
  全一致 min 35.70dB を確認・再実行では再現せず)。フル経路は検出ゲート無しで
  黙って出るのに対し、fastPath は verifyAssembled が同種異常を弾いて自動退避
  する=既定化の追い風材料。詳細は perf.md フェーズ11
- **既定化: GO** — `config.yaml` の `render.fastPath: true` に変更(**コード側の
  省略時既定は false のまま**=`resolveFastPathCfg` は不変・config を持たない
  環境では従来どおりオフ。test/config.test.ts の既定ピンもそのまま有効)。
  「数収録の実運用」は在庫が実収録1本のため、実収録1本+4合成変種の深い検証+
  自動フォールバック+1行切り戻しで代替判断した。**今後の数収録は final を
  ざっと目視し、違和感が出たら `fastPath: false` に戻して収録フォルダを保全→
  §8 に追記**する運用とする
...
- **2026-07-14 P5-4 完了(inserts の span 化=最後の全編フォールバック解除)**(Opus 設計
  ...
  - **教訓(恒久)**: **映像の PSNR は音声のズレを一切検出しない**。音声に触る
    ...
    検証すること。
- **2026-07-14 P6 完了(検収セッション)= プログラムの当初ゴール到達**。
  実収録(BGM×3)で full 202.6s → fast 102.2s/92.5s(約2.1倍速)、リソースは
  フェーズ9手法で初実測(併走ピーク 6.7GB→2.7GB・空きメモリ最小 23MB→1.8GB)、
  等価性は frame 数・PSNR(42.52dB/min 33.04)・LUFS(差 0.0)で確認。
  `config.yaml` の `render.fastPath: true` を既定化(コード側省略時既定は
  false のまま)。**副産物としてフル(Remotion)経路の非決定 +1 frame フレークを
  観測**(2回中1回 6302f・frame 3567 付近に重複1枚・再現せず。フル経路は
  検出ゲート無し)。数値・分析の正は docs/perf.md フェーズ11。残タスクは
  P5 後続(カラオケ word / anim テロップ / 動画素材 overlay / ショート展開。
  すべて自動フォールバックで安全に SLOW/フル退避するためバックログ扱い)。

---

## 9. 次セッションの着手手順(P0〜P6 完了 → **プログラムの当初ゴール到達。残は P5 後続のバックログと実運用ウォッチ**)

**現状(2026-07-14)**: `render.fastPath: true` が config.yaml の既定。数値の正は
docs/perf.md フェーズ11(cold 2.1倍速・併走メモリ 6.7GB→2.7GB・PSNR/LUFS 等価)。

**実運用ウォッチ(次の数収録でやること)**:

1. 通常どおり編集して render し、final をざっと目視(テロップ位置・演出の
   タイミング・末尾の A/V 同期)。発動状況は render の1行ログ
   (`render 高速パス: FAST n / SLOW m(被覆 x%, 音声 mode)`)で確認できる
2. 違和感が出たら `config.yaml` の `render.fastPath: false` で即切り戻し、
   収録フォルダを保全してから §8 に症状を追記する(fast/full の突き合わせ手順は
   §5 P3.5 の「検収の再現手順」を再利用)
3. 問題なく数本続いたら、その旨を §8 に一行追記してプログラムをクローズする

**P5 後続に着手する場合(バックログ。優先度は低い)**: §10 → §5 P5 の「残」
(カラオケ word の区分静的 PNG 列 / anim テロップの窓分割 / 動画素材 overlay の
span 化 / ショート展開)→ §8 の順に読む。どれも現状は自動で SLOW/フル退避する
ため、着手動機は「被覆率をさらに上げたい収録が実際に出たとき」でよい。

**既知の観測(未着手の調査タスク)**: フル(Remotion)経路に非決定の +1 frame
フレークがある(P6 検収で 2回中1回・6302f・詳細は perf.md フェーズ11)。fastPath
既定化により通常運用では踏まないが、フル経路にも verifyAssembled 相当の
フレーム数ゲートを足す価値はある(着手する場合は §5 P6 の副産物の項を参照)。
