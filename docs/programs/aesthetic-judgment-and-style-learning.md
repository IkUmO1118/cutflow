...
> **一言**: 精度母艦(`docs/programs/edit-precision-program.md`。以下 **精度母艦**)は
...
1. **候補スコアリングの事前分布**(「このチャンネルは平均ショット 2.4s → そのペースへ寄せて切る重み」)。
   **実装済(SD-T4・2026-07-12)**: `plan.styleProfile`(既定オフ)が
   `src/lib/styleInjection.ts` の `renderStyleProfileBlock` で cut/caption/structure
   の3面だけを compact policy(日本語圧縮 summary。raw JSON ではない)へ畳み、
   `plan`/`plan --cuts-only` のプロンプトへ `{{perception}}` 直後・最弱の最後尾
   ブロックとして注入する。off はバイト等価、profile 不在/破損は警告して優雅に
   劣化(前提エラーにしない)。番号選択方式は不変(精密な値・タイムスタンプは
   生成させない)。audio(§7 の pace 由来 `pauseToleranceSec` 等)は v1 では
   注入しない(profile v1 に無い値のため。cut 判断に直接効く3面に閉じた)。
2. **few-shot 判断例**(自コーパスから「似た過去状況ではこれを残した」を提示)。
   **v2 拡張点として明示 defer**(SD-T4 設計書 §1.1)。cold-start N=1 では
   few-shot に載せる「似た過去状況」が薄く、承認済みプロジェクトが複数溜まって
   から着手する(自信作が育つ経路。SD-T2 agreement と同じ育ち方)。
3. **自己検証 assert の閾値**(§5.1 — プロファイルが assert のパラメータになる)。
   **実装済(SD-T1)**: `style-check` が profile→距離(J→D 変換)。
...
| Q9 | **スタイル注入**(候補スコア・few-shot・assert 閾値) | §5.3 / 新設 T 系 | 全軸 | 経路1(候補スコア)実装済(SD-T4)/ 経路2(few-shot)v2 defer / 経路3(assert 閾値)SD-T1 済 |
...
| **SD-T4** | profile→判断注入 | 候補スコア→few-shot、cut 経路先行。opt-in/sticky。効果は T1 assert で測る | 旧 Q1–Q4/Q6/Q7/Q9・reference P2 | **完了(経路1・PR #31・2026-07-12)** |
...
- **2026-07-12(SD-T4 実装完了・PR #31。ロードマップの活性 SD を完走)**: `plan.styleProfile`(opt-in/sticky・既定 off)を
  実装し main へマージ。SD-T0 の style profile を **compact な soft prior**(§5.3 経路1)に落として cut 判断 LLM
  (`plan` / `plan --cuts-only`)のプロンプトへ注入する。確定した実装事実:
  - **off でバイト等価**を `plan.perception` と同じ理屈で担保: テンプレの `{{rules}}{{perception}}` を
    `{{rules}}{{perception}}{{styleProfile}}` にし(隣接・区切り無し)、off で両方 "" に置換=導入前と1バイト差なし。
    証明 = `test/perception.test.ts` の git diff が**空**(byte-identical)で無修正緑・テンプレ diff は `{{styleProfile}}`
    追加のみ。`renderPrompt` に第7引数 `styleProfile=""` を末尾追加(既存呼び出しは "" でバイト等価)。
  - **compact policy = cut / caption / structure**(profile v1 が値を持つ軸に閉じる。audio/演出/素材は v2 defer)。
    閾値→日本語文言の決定論写像(`src/lib/styleInjection.ts`。数値経路に LLM 無し)。cold-start N=1(conf 0.25〜0.33)は
    各行に「弱い(参考程度)」を明示し LLM を誤誘導しない。
  - **番号選択方式を維持**(§8 #5): 出力スキーマ不変・prior として注入するだけ。prompt ヘッダに「番号選択の重み付けに
    だけ使い精密な値・タイムスタンプは生成しない・brief と矛盾したら brief 優先」を明記(§6.2 brief 上書き)。prior は
    brief→rules→perception→styleProfile の最後尾=最弱に配置。profile 不在/破損は warn+null で空注入(plan は止まらない)。
  - **注入先は cut 判断経路のみ**(非cutsOnly plan / generateCutsOnce / runCutsLoop の iter===0)。critique 反復・
    meta.md・plan-shorts.md には注入しない(v1 defer。回帰テストで no-op を固定)。経路2(few-shot 判断例)は
    cold-start では薄いため v2 defer(SD-T2 と同じ育つ経路)、経路3(assert 閾値)は SD-T1 で実装済。
  - 実測(決定論): 実 2026-07-02 profile から compact policy が正しく導出され plan.md プロンプトの正しい位置
    (perception と判断基準の間)に注入。off で残骸なし・status off/on 正常・編集ファイル/profile 書込なし。
    品質効果(注入 on/off で style-check の pace 逸脱が縮むか)は SD-T1 の運用測定(scratch × K 回)=マージゲートではない。
  - **ロードマップの活性 SD(T0/T1/T4)を完走**。残るは **SD-T2(agreement・承認済み ≥2〜3 本まで保留)**・
    **SD-T5(接地 VLM 審美 judge・vision route 前提)**・~~SD-T3~~(第三者エミュレーション・park)。次に承認済み自信作が
    ≥2〜3 本溜まったら SD-T2 を起こし、few-shot(経路2)と cross-project 実分散(T1 の帯の学習値化)も育つ。
