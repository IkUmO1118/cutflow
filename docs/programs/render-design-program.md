...
> 状態: **P0〜P3実装・検収完了**(2026-07-15、初版 2026-07-14)。単発の feature 設計ではなく、
...
続編**)/ `docs/perf.md`(フェーズ0〜12。数値の出所)/
...
- P2: SLOW/フル経路の design 描画軽量化を実測し、効果がなく棄却。
  renderStill製静的資産はP1 FAST基底として採用
...
## 2. P3実装前の確定事実(2026-07-14。履歴ベースライン)

この節はP1/P3着手時の因果関係を残す履歴であり、現在仕様ではない。特に
plainの門前払いと高速パス不発は2026-07-15のP3で解消済み(§8-5)。

### 2.1 当時、高速パスが全停止していたメカニズム(履歴)
...
- P3前の `resolveDesign` は `hasCamera`(manifest.video.cameraRegion の有無)で
  **plain を門前払い**しており、`DesignProps.camera` も必須だった。これは
  現在仕様ではなく、P3でcamera optionalへ変更済み。
...
- **当時はplainの高速パスが不発だった**: plain の cut.mp4 は恒等クロップで最初から
...
render <dir> ────────┼─ chunk 差分(P0 で design を globalKey へ)
    ...
    基底の組み立てが3形態:          Main の legacy CSS design 描画
      composite: cut.mp4 素通し      (背景Img+native角丸+CSS box-shadow)
...
### P1 — FAST 基底の design 対応 【状態: 完了 / 2026-07-15】
...
### P2 — SLOW/フル経路のバックプレート統一 【状態: 実測棄却 / P1用静的資産基盤のみ採用 / 2026-07-14】
...
**実測結果**: renderStill製のbackdrop/screenMask/cameraShadow/cameraMask生成・
cache・props供給はP1 FAST基底として採用した。一方、Mainで同資産を使う
軽量化は§8-3の制御実測で効果がなく、legacy CSS描画へ戻した。

### P3 — plain への展開(design 部分適用+高速パス解禁) 【状態: 完了 / 2026-07-15】
...
**実装結果**:
1. `DesignProps.camera`をoptional化。plainはbackground+screenだけを解決し、
   channel共通camera設定は警告せず無視する。OBSの解決済みJSONは不変。
2. screen geometryは縦横共通の既存式を維持。1920x1080は
   `{x:100,y:22,w:1720,h:968}`、1080x1920は
   `{x:100,y:266,w:880,h:1564}`。値と出力内rectを厳格検証する。
3. repo/absolute背景はplainでも`render.design/`へ供給し、recording相対は
   素通し、missingは背景色、disabledは無副作用とした。
4. `isPlainIdentityBase`はlayout/design/camera不在とcanvas/screenの実寸一致で
   証明する。design基底はcamera無しならbackdrop+screenMaskの2-roleだけを要求し、
   ffmpeg graphもscreen branchだけを構築する。
...
| renderStill 製FAST資産と Main.tsx のlegacy描画差(将来のスタイル変更で乖離) | PSNR の谷が FAST/SLOW 境界に出る | 同じCSS定数/geometryから資産を生成し、P1の全境界±10 frame PSNRを必須にする |
...
| plain の縦出力で design 既定値が破綻 | resolveDesign の余白エラー/歪んだパネル | 縦横共通式とrect境界検証を採用。1080x1920の解決値・実写frameをP3ゲートで確認済み |
| 高周波素材でfull/FASTのcodec差が目立つ | synthetic stressのPSNRが30dB未満 | geometryではなくfull(libx264)/FAST(VideoToolbox)の固定bitrate差。実収録ゲートを維持し、必要ならencoder/bitrate統一を別施策で測る |
...
- **実測は実装後にコーディネータが独立検証する**。各フェーズの cold render、
  PSNR、frame/fps/duration、音声、fallback/cache保持を実装報告と分離して判定する。
...
### 8-3. 2026-07-14 P2実測棄却 — Mainはlegacy描画を維持

- commits `4b47a05` / `0d6b5ea` で4静的資産の内容アドレス式cacheと
  render/frames/editorへの供給を実装した。背景bytesもkeyに含み、partial cacheや
  生成失敗はattachせずfallbackする。この基盤はP1 FASTのため維持する。
- 初回asset-mask版は正確性を満たしたが性能不合格: 実収録全編はlegacy
  7:01 / asset 7:24、1000-1299 frameの順序入替3回中央値はlegacy 17.86秒 /
  asset 18.78秒(assetが5.2%遅い)。全6301 frame PSNRはfinite平均60.29dB /
  最小53.27dB / 30dB未満0、wipeFull区間は完全一致だった。
- maskをnative rounded clipへ戻したhybrid(`abc938f`)も、5回交互測定で
  legacy 17.88秒 / hybrid 17.91秒と改善しなかった。backdrop-onlyの要因分離も
  正の効果を証明しなかった。静的box-shadowがChrome内で再利用されている可能性は
  あるが、原因は断定しない。
- commit `c435247` でMainを背景Img、native角丸、CSS shadowの完全legacy描画へ
  戻した。assets有無の実render PNGはバイト一致。4asset生成/cache/attachは
  P1基盤として残し、P2の速度改善は実測棄却してP1へ進む。

### 8-4. 2026-07-14 P1 checkpoint — 正確性完了、性能は内訳再測定中

- commits `0ee872d` / `3a22a94` / `9053624` で、基底能力ゲート、design用
  ffmpeg graph、activationと限定cleanupを実装した。`fastPlan`は変更せず、
  design/caption/overlay/annotation cacheは成功・失敗cleanup後も保持する。
- scratch `/private/tmp/cutflow-render-design-p1` のcold renderは
  `FAST 2 / SLOW 2`、被覆81.6%、音声`bgm-mix`で成功した。spanは
  SLOW `[0,356)`、FAST `[356,686)`、SLOW `[686,1490)`、
  FAST `[1490,6301)`。6301 frame / 30fps / 210.033秒でverify成功。
- P2時点の`legacy-full.mp4`との初回比較は、blurのtype/strengthとcaption既定値が
  current propsと異なっていたため合否から除外した。同一props SHA-256
  `5025389d...b968957`でcurrent fullを再生成し、全編PSNRはffmpeg集計平均
  39.886dB、per-frame finite平均42.807dB、最小30.82dB、30dB未満0だった。
  SLOWは平均59.377/min49.3、FASTは平均40.216/min30.82。境界356/686/1490の
  各±10 frame最小は36.93/42.56/40.47dBで、新しい境界谷はない。
- screen FAST領域はmin30.48/30dB未満0。camera領域は高motionのframe 593付近で
  局所的に30dB未満5 frameがあるが、同frameの全画面はmin30.82を維持し、境界には
  集中しない。局所値を隠さず記録し、全画面の受入基準で正確性合格とした。
  音声は比較対象とともに-14.2 LUFS、design 4assetを含むcache残存も確認した。
- cold壁時計は既定`chunkSec:15`で202.8秒、直後の8分full render後に行った
  `chunkSec:0`は熱状態が混入して224.4秒となり再現しなかった。単体測定では
  keyframe probe 9.916秒、video carve 0.043秒、audio抽出0.037秒。
  commit `3366d0b`で挙動を変えないphase timingを追加した。
- 冷却後の計時で、segment全体162.2秒に対してverify20.2秒、seed probe10.0秒と、
  同じ動画を3回全走査していたことを確定した。packet数/stream `nb_frames`だけへ
  弱める案は破損frameを通しうるため棄却。commit `5047aa5`でdecoded frameの
  countとkeyframe ordinalを1回のprobeで得て、検証成功時だけseedへ引き渡す方式を
  採用した。B-frame fixtureと30000/1001 fixtureでもordinal/countを固定した。
- 最終cold run(`/private/tmp/cutflow-render-design-p1-final`、開始load
  1.76/2.41/3.54、throttling 0)は、design準備1.6秒、SLOW 19.4+42.6秒、
  FAST 6.3+93.3秒、segment全体161.9秒、audio3.6秒、verify10.9秒、
  FAST合計176.5秒、seed0.1秒、CLI内部178.2秒/外部178.39秒だった。
  `FAST 2 / SLOW 2`、被覆81.6%、`bgm-mix`を維持し、第一目標150〜180秒を達成。
  最終出力も同一props hash、6301 frame / 30fps / 210.033秒、全編PSNR
  平均39.886dB/min30.822dB/30dB未満0、音声-14.2 LUFSを再確認しP1完了とした。

### 8-5. 2026-07-15 P3完了 — plain designとFAST基底を一級化

- 設計は`docs/plans/2026-07-14-design-plain-design.md`のD1〜D7を採用。
  camera optional、縦横共通geometry、plain camera設定の無警告ignore、
  capability優先順`composite -> design -> plain-identity -> false`を確定した。
  `canBurnWipe`と`fastPlan`は変更せず、責務を分離した。
- commits: `64b963d`(camera optional/plain resolve)、`82b57fe`
  (背景供給/editor/契約)、`3b5e8c5`(plain identity/design FAST基底)、
  `1a21b97`(背景cache timing testの安定化)。
- 実収録由来6秒scratchの4ケースは全て180f/30fps/6.000s、音声-14.2 LUFS。
  no-designは横/縦ともfull 5.5秒からFAST 1.9秒。designは横6.6秒から3.3秒、
  縦6.6秒から3.2秒。全編PSNR(avg/min/30dB未満)は横no-design
  44.8958/44.42/0、横design 44.0856/43.67/0、縦no-design
  45.8423/45.40/0、縦design 46.1911/45.77/0。panel cropも横design
  43.3445/42.99、縦design 44.7723/44.41で合格した。
- 横geometry`{100,22,1720,968}`、縦`{100,266,880,1564}`、camera/wipe無し、
  repo背景・角丸・影を目視確認。`render.fast/design`をfileで塞いでEEXISTにした実測も
  `design基底asset不足(backdrop/screenMask)`理由で通常Remotionへ退避し5.2秒で
  成功した。横design代表runの親プロセスmax RSSはfull 745,340,928B / FAST
  677,150,720Bだったが、子プロセスを含まないためプロセスツリー総量とは扱わない。
- 縦design+blurの`FAST 1 / SLOW 1`(被覆50%、境界frame 90)は序数PNG比較で
  全編finite平均52.0523dB/最小43.62dB/30dB未満0、境界80〜100は
  平均54.2748dB/最小44.30dB/30dB未満0。直接framesyncはB-frame timestamp表現で
  境界停止したため、decode後序数PNGを正式値とした(詳細は`docs/perf.md`フェーズ12)。
- `npm run typecheck`成功、`npm test` 1565/1565成功。高周波synthetic stressは
  codec差を増幅してplain 25.88/design 29.46dBだったため正式判定から除外した。
  geometry不一致ではなくfull(libx264)/FAST(VideoToolbox)の固定bitrate差であり、実収録由来の
  正式測定を合否根拠とする。これはcodec依存の残リスクとして保持する。

---

## 9. 実装時のコールドスタート前提(履歴)

以下はP1〜P3実装セッションへ渡した当時の指示を再現する履歴であり、P3完了後の
未着手タスクや現在の委任指示ではない。
...
- ブランチは `render-design-program`。main と比較してからコードの存否を断定する。
