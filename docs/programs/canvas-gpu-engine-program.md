...
> 状態: **P0 完了（2026-07-22・実機で良好）。P1/P2 は条件付き保留（§8 の再開トリガ待ち）。** 目的は
...
- **現在地（2026-07-22）**: **P0 完了。実機で「流しで非常に滑らか」＝脱ガタつきの狙いは
  達成**（実測はユーザー実機。出来は良好）。**P1/P2 は条件付き保留**にした（§8 の再開トリガが
  実作業で出るまで backlog）。この母艦は「何を作り替え、何を絶対に触らないか」を固定するための正本。
- **確定した方向**: CutFlow が土台のまま、描画エンジンだけを Remotion+`<video>`
  から canvas/GPU コンポジタへ作り替える。OpenCut は設計図＋部品供給元（MIT）。
  データモデル・CLI・AI の脳・承認は不変（§1・§7）。
- **次の一手（なし＝様子見）**: P0 で主症状（カット境界 seek ヒッチ）が取れたため、**P1 は
  急がない**。§8 の再開トリガ（再ベイク待ちが編集リズムを崩す／オーバーレイ過密でカクつく／
  preview≠final の手戻り）が実作業で顕在化した時点で、P0 同粒度の P1 設計書を起こして着手する。
  それまで P1/P2 の §8 未決（mediabunny 流用/P2 射程/byte 決定性）は先送り。
- **P0 の恒久メンテ注記**: P1 が保留＝**P0 は当面 load-bearing**。撤去前提だが放置はしない
  （実機で出た穴は塞ぐ。例: 2026-07-22 の「再ベイク完了スワップ時に一時停止プレビューが黒くなる」
  ＝Player remount 後の再 seek 欠落を修正。§9）。保留が恒久化するなら P0 を「暫定」から
  「恒久の土台」へ格上げする判断を §9 に追記する。
...
- **実機で発覚した穴 → 修正（2026-07-22）**: 再ベイク完了で proxy→preview-cut にスワップする瞬間、
  一時停止中のプレビューが黒くなる（手スクラブで直る）。原因＝`videoVersion` bump で Player を
  `key` remount するが remount 後に現フレームへ seek し直さず、新 `<video>` が最初の seek まで
  描画しない。修正＝`App.tsx:1500` 付近に remount 後 `seekTo(playhead.get()*fps)`（rAF×2）を追加。
  baked ファイル自体は正常（出力秒フレーム＝黒でない・尺も cutplan と一致）。tsc パス・要 editor 再起動で反映。

### P1（本丸・条件付き保留）— プレビューを WebCodecs + OffscreenCanvas コンポジタへ
- **状態: 条件付き保留（2026-07-22）**。P0 で主症状が取れたため急がない。§8 の再開トリガが
  実作業で出た時点で、P0 同粒度の P1 設計書を起こして着手する（それまで backlog）。
...
- **[完了 2026-07-22] P0**（ユーザー判断で挟んだ・§9）。実機で良好＝主症状の脱ガタつき達成。
- **[保留 2026-07-22] P1/P2 は条件付き保留**。P0 で体感の主症状が取れたため急がない。下記の
  **再開トリガ**が実作業で顕在化したら、P0 同粒度の P1 設計書を起こして着手する:
  - **再ベイク待ちが編集リズムを崩す**: カットを連続で詰める最中、再ベイク中の seek-over-proxy
    フォールバック（dual-path の仕様）でガタつく瞬間が編集の妨げになる。
  - **オーバーレイ過密でカクつく**: テロップ・素材・zoom・wipe が多い場面の再生が詰まる
    （P0 はメインスレッド React 合成を触っていない＝ここは P1 でしか取れない）。
  - **preview≠final の手戻り**: 最終 render とプレビューのズレで作り直しが出る（P2 の担当）。
  - どれも出ないなら P1 は起こさない。P0 を load-bearing な暫定として維持する（§0・§9）。
...
- **2026-07-22（P0完了・良好）**: 実機でプレビューを確認し「流しで非常に滑らか」＝カット境界
  seek ヒッチの脱ガタつきを達成。実機で1件の穴（再ベイク完了スワップ時に一時停止プレビューが
  黒くなる＝Player remount 後の再 seek 欠落）を発見し `App.tsx:1500` 付近で修正（remount 後
  `seekTo(playhead.get()*fps)`・rAF×2）。baked ファイル・尺は正常と確認済み。
- **2026-07-22（P1/P2 を条件付き保留・ユーザー判断）**: P0 で体感の主症状が取れたため、数週規模の
  P1（canvas エンジン本丸）/P2 は**急がない＝条件付き保留**にする。再開は §8 の再開トリガ（再ベイク
  待ちが編集リズムを崩す／オーバーレイ過密でカクつく／preview≠final の手戻り）が実作業で顕在化した
  時のみ。トリガが出ない限り P1 は起こさず、**P0 を load-bearing な暫定として維持・穴は塞ぐ**。
  保留が恒久化した場合は、P0 を「撤去前提の暫定」から「恒久の土台」へ格上げする判断を別途本ログへ追記する。
- **未記入**: P1/P2 の着手判断・go は、再開トリガ発生時に §8 の残る未決事項を潰してから本ログに追記する。
