...
- LLM を使うのは `plan`(意味カット・章立て)だけ。**デフォルトは `claude` CLI**
  ...
  `llm.model` を設定します。
  - **既定の `claude-cli` は `claude` CLI(Claude Code)本体のインストールと
    認証が前提**です。未導入だと `plan` 段で `コマンド 'claude' が見つかりません`
    で止まります。`claude --version` が通り、`claude` にログイン済みであることを
    確認してください(不要にするには上記の `llm.backend: api` に切り替え)。
...
- **`コマンド 'claude' が見つかりません`(plan / run で停止)** → 既定の LLM
  バックエンドは Claude Code の `claude` CLI です。Claude Code をインストールして
  `claude` にログインするか、従量課金 API に切り替える(`.env` に
  `ANTHROPIC_API_KEY`、`config.yaml` の `llm.backend: api` と `llm.model`)。
