...
| SD | ねらい | 中身 | 対応 | 状態 |
|---|---|---|---|---|
| **SD-T0** | 任意パス→profile(測定の基質) | `style-profile --from <path>`。素の動画=統計のみ / 収録=統計+補正デルタ。channel `style.probe/profile.json` へ provenance 付き集約 | reference P0・`learn` 一般化(採掘モード) | **次の一手** |
| **SD-T1** | J→D 変換 | profile→距離 assert(warn・学習分散帯)。§2 次元表の再分類。**cold-start でも測れる測定面** | 旧 A3/A5・精度母艦 X3 | T0 の直後 |
