...
| ペース | 冗長/言い直し/脱線は切り、見せ場は残す | 目標尺・累積尺・話速(C4/C5)。**プロファイルで D 化可** | J→D |
...
| ラウドネス | 全体が目標 LUFS に収まる | integratedLufs(bgm-fit 実装済)。**目標 LUFS をプロファイルから** | **D** |
...
| 次元 | プロファイル無し | プロファイル有り |
|---|---|---|
| ペース | J(目標尺が主観) | **D**:ショット長分布の KS 距離 |
| caption 密度/位置 | J | **D**:coverage 比・位置ヒストグラムの距離 |
| BGM loudness/切替頻度 | 一部 D | **D**:目標 LUFS・章あたり切替回数の距離 |
| 演出密度 | D(閾値ハードコード) | **D**:閾値が config 固定でなく**学習値** |
| 構成(フック長・CTA) | J | **半 D**:構成テンプレ一致 |
...
| **SD-T1** | J→D 変換 | profile→距離 assert(warn・学習分散帯)。§2 次元表の再分類。**cold-start でも測れる測定面** | 旧 A3/A5・精度母艦 X3 | **着手中** |
| **SD-T4** | profile→判断注入 | 候補スコア→few-shot、cut 経路先行。opt-in/sticky。効果は T1 assert で測る | 旧 Q1–Q4/Q6/Q7/Q9・reference P2 | T1 の後 |
