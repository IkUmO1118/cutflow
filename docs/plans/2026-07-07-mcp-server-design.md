...
- **tool レジストリは `ToolDef[]` 配列 + name→handler の Map**。F1/F2/F3 の新
  コマンドは「配列に1件 push + handler 1関数 + AGENTS_CONTRACT.md に1行」で足せる。
...
2. **`agentsMd.test.ts` の規約と自然に整合**。同テストは登録コマンド名 `<name>` ごとに
   AGENTS_CONTRACT.md が `` `<name> <dir>` `` を含むことを要求する。`mcp <dir>` はこの規約に素直。
...
いずれも `makeTools` の配列に1件足す + AGENTS_CONTRACT.md §11 に1行足す + docs で触れるだけで
...
### 信頼モデル宣言(AGENTS_CONTRACT.md §11 / docs にも明記する)
...
| `AGENTS_CONTRACT.md` | §10 コマンド表に `` `mcp <dir>` `` を1行追加。加えて **§11「MCP tools」新設**(露出 tool 一覧 + §6 の信頼モデル宣言)。BYO-AI の契約面なので AGENTS_CONTRACT.md が正しい載せ先 | `test/agentsMd.test.ts` が全登録コマンド名を要求(§下記の落とし穴) |
...
`.command("mcp ...")` を検出したら AGENTS_CONTRACT.md に文字列 `` `mcp <dir>` `` があることを
要求する。**cli.ts に `mcp` を足したコミットと同じコミットで AGENTS_CONTRACT.md を更新**
しないと `npm test` が落ちる(T4 と T5 の順序に注意、または1コミットに束ねる)。

MCP は Claude 非依存で書くこと(`test/agentsMd.test.ts` は AGENTS_CONTRACT.md に
...
- **(a) 触る**: `AGENTS_CONTRACT.md`(§10 に `mcp <dir>`、§11「MCP tools」新設),
...
- **(a) 触る**: `src/mcp/tools.ts`(`makeTools` に ToolDef を1件 push +
  handler 1関数), `AGENTS_CONTRACT.md` §11(1行), `docs/usage.md`(1行),
