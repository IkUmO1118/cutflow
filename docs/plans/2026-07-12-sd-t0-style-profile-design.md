...
- `density = captionDensityLabel(coverageRatio)`: `null→null` / `< 0.3 → "low"` /
  `< 0.6 → "medium"` / `≥ 0.6 → "high"`。
- **位置**: 各 caption の y を、`caption.pos?.y` があればそれ、無ければ**その track の既定 y**
  (`proj.overlays.captionTracks` から `track` 一致の `y`。それも無ければ skip)。`canvasHeight =
  proj.source.video.screenRegion.h`。バケット: `y < H/3 → top` / `H/3 ≤ y < 2H/3 → center` /
  `y ≥ 2H/3 → bottom`。`positionHistogram = {top,center,bottom}` の件数。
...
- **分布統計はプーリング**(平均の平均にしない): 全観測の `shotDurations` を連結してから
  `mean/median/percentile`。captions も `captionOutIntervals`/`captionDisplaySecs`/`captionYs` を連結。
...
- **structure.segments**: 入力1本のときだけそのまま。N>1 は timeline を混ぜられないため `segments=null`
  にし、`chapterCount`/`hookSec` は平均、`ctaLikely` は多数決(過半 true→true)。
