...
> **ゲート4の充足(2026-08-02 追記)**: 実例が出た。S0-T2 が `av --range` の
> **解釈**だけを挿入込みにし、測定結果を出力秒へ**戻す**写像を keep のみの
> 経過秒(elapsed)のまま残したため、挿入があると `av.probe/` の全出力が
> 挿入尺ぶんズレていた。`plan-effects` は `effectAnchors.ts:244` の挿入なし
> 逆変換で二重にズレる。詳細と是正 =
