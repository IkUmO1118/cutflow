...
できるようにする。(2) リポジトリ直下に **AGENTS_CONTRACT.md**(emerging standard の agent 向け
...
- **AGENTS_CONTRACT.md**(リポジトリ直下・英語・Claude 非依存):能力/不変条件/承認境界/
...
> JSON Schema も AGENTS_CONTRACT.md も**新しい真実を宣言しない**。types.ts(構造)・
...
キー」になる。**却下**。ファイルとスキーマの紐付けは AGENTS_CONTRACT.md の命名規約
...
### 論点3: AGENTS_CONTRACT.md の内容と生成 → **決定: 手書き Markdown(英語・Claude 非依存)を正とし、列挙部分をドリフト検知テストでコードに縛る。CLAUDE.md は併存し AGENTS_CONTRACT.md を参照**
...
から吐く)は**採らない**:AGENTS_CONTRACT.md は稀にしか変わらない1枚の Markdown で、生成器を
...
**Claude 非依存の言語**: AGENTS_CONTRACT.md には「`claude -p`」「Claude Code」等の Claude
固有語を**持ち込まない**。「any coding agent」「run `node src/cli.ts validate <dir>`」
のように backend 非依存で書く。言語は**英語**(AGENTS_CONTRACT.md は emerging standard で
codex 等が読む前提。英語が最も広く parse される)。日本語の運用ニュアンスは
CLAUDE.md / usage.md に残す。AGENTS_CONTRACT.md は翻訳ではなく**契約の distillation**。

**CLAUDE.md との関係**: **併存。AGENTS_CONTRACT.md を「契約(何が編集可能か・不変条件・
承認境界・コマンド)の正」とし、CLAUDE.md は「Claude Code 固有の運用ニュアンス」を
保つ。** CLAUDE.md 冒頭に AGENTS_CONTRACT.md への1行ポインタ(「機械可読・エージェント非依存の
契約は AGENTS_CONTRACT.md。本書は Claude Code 向けの運用補足」)を足す。200行を複製しない:
AGENTS_CONTRACT.md は列挙 + 規約の要点 + リンクに絞り、詳細(backups の復元手順・frames の
stale 罠等)は usage.md/CLAUDE.md へリンクで逃がす。二重に持つ列挙(ファイル一覧等)は
**同じコード定数へ両方ピン留め**するので、片方だけずれることはない。

---

### 論点4: AGENTS_CONTRACT.md の配置と per-project の是非 → **決定: リポジトリ直下 `AGENTS_CONTRACT.md` のみ。収録フォルダの per-project マニフェストは足さない**

- **リポジトリ直下 `AGENTS_CONTRACT.md`**(emerging standard。codex 等が既定で探す位置)。
- **per-project(収録フォルダ)マニフェストは足さない**:収録フォルダの「契約」は
  ...
  AGENTS_CONTRACT.md は「per-project の状態は `describe --json <dir>` を読め」と誘導する
  (スコープを広げない)。

---

### 論点5: 検証(codex 等が本当に使えるか) → **決定: 3層で担保。(a) fixture/bench が schema に valid、(b) AGENTS_CONTRACT.md の列挙が code 定数と一致、(c) 外部バリデータ + 実エージェントの手動実測**
...
2. **(b) 契約が実挙動と一致**(自動):AGENTS_CONTRACT.md とスキーマの列挙(ファイル一覧・
   deny 一覧・id 接頭辞・enum・コマンド)を code 定数へピン留め(§論点6-a/c)。
   validate.ts の enum・files.ts の分類が変われば落ちる=「AGENTS_CONTRACT.md が validate の
...
3. **(c) 手動実測**(実装後・人間):
   ...
   - codex 等 Claude 以外のエージェントに AGENTS_CONTRACT.md だけ渡し、収録フォルダを編集させ、
...
`AGENTS_CONTRACT.md`(第5点)**を追加する、と明記。
...
- **(d) AGENTS_CONTRACT.md ピン留め**(`test/agentsMd.test.ts`):AGENTS_CONTRACT.md 本文を読み、
...
schemas/
  ...
  examples/
    ...
    …(各編集ファイルぶん)
AGENTS_CONTRACT.md                     # リポジトリ直下・英語・Claude 非依存
test/
  ...
  agentsMd.test.ts            # AGENTS_CONTRACT.md 列挙 ↔ code 定数のピン留め
```

`approvals.json` は**第3カテゴリ**(AI 非書き込み)。スキーマは v1 では作らない
(全単射テストを `EDITABLE_FILES` に単純化するため)。AGENTS_CONTRACT.md の承認境界節で
...
### T5: AGENTS_CONTRACT.md(`AGENTS_CONTRACT.md` + `test/agentsMd.test.ts`)

- **触る(新規)**: `AGENTS_CONTRACT.md`(§論点3 の目次。英語・Claude 非依存。編集ファイル表は
...
- **テスト方針**: AGENTS_CONTRACT.md を読み、`EDITABLE_FILES` / `GENERATED_FILES` +
...
- **触る**: `CLAUDE.md`(「コードを触るとき」の3点セットへ `schemas/*.json`・
  `AGENTS_CONTRACT.md` を追加。冒頭へ AGENTS_CONTRACT.md ポインタ)。`docs/usage.md`(「機械可読契約」
  節=schemas の場所・命名規約・`$schema` を注入しない旨・外部バリデータの使い方・
  AGENTS_CONTRACT.md への言及)。`src/types.ts`(各スキーマ対応のコメントに「`schemas/<file>.
...
1. **純粋な新規成果物 = 既存挙動バイト等価**: `schemas/` と `AGENTS_CONTRACT.md` の追加で、
...
3. **ユーザーデータ不可侵**: 収録フォルダの編集ファイルに `$schema` 等のキーを
   注入しない。schema/AGENTS_CONTRACT.md は収録フォルダの外(リポジトリ資産)。
4. **契約は既存の単一の出所の射影**: schema/AGENTS_CONTRACT.md は新しい真実を宣言せず、
   `types.ts` / `validate.ts` / `files.ts` / `ids.ts` / CLI 登録へ**テストでピン留め**。
   食い違ったら `npm test` が落ちる。
5. **Claude 非依存**: AGENTS_CONTRACT.md に Claude 固有語(`claude -p` / Claude Code)を
...
- **やる**: 8編集ファイル + apply パッチの JSON Schema(draft 2020-12・共有 `$defs`・
  checked-in)、AGENTS_CONTRACT.md(英語・Claude 非依存・列挙ピン留め)、ドリフト検知テスト
  1系統、5点セット規約への反映。
- **やらない**: types からの codegen、MCP サーバ/tool 本体、複数バックエンド実体化
  (Later)、ユーザー JSON への `$schema` 注入、per-project マニフェスト、
  approvals/生成物のスキーマ、AGENTS_CONTRACT.md 生成コマンド。ただし **apply コマンド
  (Feature 4)への参照は AGENTS_CONTRACT.md の「書き込み経路」節に含める**(推奨経路として
