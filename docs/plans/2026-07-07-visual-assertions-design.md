...
| `AGENTS_CONTRACT.md` | (1) §10 コマンド表に `` `assert <dir>` `` を追加(**必須**: `test/agentsMd.test.ts` が cli.ts の全 `.command()` 名を AGENTS_CONTRACT.md 命令表に要求する)。(2) §3 付近に assertions.json を「意図宣言ファイル(第3の手書きカテゴリ・render に影響しない)」として1段落追記 |
| `src/lib/files.ts` | **無変更**(論点1: "other" 扱い)。`test/files.test.ts` も無変更で green |

> ドリフトテストのピン留め要注意点(実装者向けチェックリスト):
> - `test/agentsMd.test.ts`: `assert` コマンド名が AGENTS_CONTRACT.md 命令表に必要。
...
### T3: スキーマ + example + ドキュメント + AGENTS_CONTRACT.md

- **触るファイル**: `schemas/assertions.schema.json`、`schemas/examples/assertions.max.json`、
  `test/schema.test.ts`(除外リストに assertions を追加)、`docs/usage.md`、`AGENTS_CONTRACT.md`。
- **テスト方針**: `test/schema.test.ts` の kitchen-sink 検証に assertions.max.json を
  ...
  `assert <dir>` を要求するので、AGENTS_CONTRACT.md 更新で green になることを確認。
