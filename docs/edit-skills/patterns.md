...
`docs/edit-skills/cut/recipes/*.md`(13分類)は「カット判断で使う語彙」全体を
...
`tool-demo` 1本だけ**(`~/Movies/framewright/2026-07-12`)。残る4型は実データ
...
| [`tool-demo`](#tool-demo) | ツール紹介・デモ | 実データ(`~/Movies/framewright/2026-07-12`。承認・レンダー済み) | 11/13分類 + blueprint(`tool-demo-arc`) | 安全/G2閉包を保つ7分類全部 + effect blueprint(`tool-demo-arc`) |
...
(`~/Movies/framewright/2026-07-12`)から重み付けした **11/13分類** を注入する:
