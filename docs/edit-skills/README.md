...
(`reasonIds.ts` / `cutPatterns.ts` / (演出は EP2 で) `effectReasonIds.ts`)から
...
重み付けるか)だけをトラックごとに分ける(`CUT_PATTERN_INJECTION` /(EP3 で)
...
| [`effects/`](effects/README.md) | **EP1 実装済み** | 演出判断(zoom/blur/annotation/none)の7分類。effectReasonId・注入・測定は EP2〜EP4 | [EP1](../plans/2026-07-21-effect-knowledge-p1-design.md) |
...
- [`blueprints.md`](blueprints.md) — アーク(`tool-demo-arc`)。収録の尺の割り方の
  スケルトン。同じアークを cut は「優勢な分類」列で、effects は(EP3 で)
  「優勢な演出」列で読む。
