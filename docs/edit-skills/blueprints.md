...
> 注入経路は `src/lib/cutPatterns.ts` の `CUT_PATTERN_INJECTION` と
> `src/lib/effectPatterns.ts` の `EFFECT_PATTERN_INJECTION`。各 pattern の
> `blueprint` がこのファイルの `##` 見出しを1つ参照する(空文字="注入なし")。
...
|---|---|---|---|---|
| フック | 冒頭 ~10% | `hook`(切らない) | `none`(`concept-talk`) | 章にしない |
| 概要 | ~15% | `restatement` / `stumble` の削り代が最大 | `none`(`already-legible` / `concept-talk`) | 1章 |
| 実演 | ~50% | `demo-wait`(残す)が支配的。`dead-air` と取り違えない | `zoom`(`tiny-target` / `focus-shift`)が集中。秘匿は`blur` | 2〜3章 |
| 設計・裏側 | ~20% | `tangent` / `gap-trim` | `none`(`motion-carries`)優先、必要な対象だけ`zoom` / `annotation` | 1章 |
| おわりに | 末尾 ~5% | `greeting`(切らない) | `none`(`concept-talk`) | 章にしない |

区間の境目は厳密な秒数ではなく、話題の切り替わり(概要→実演の移行など)で
自然に決まる。`実演` が最大の区間を占め、この中では `demo-wait` を
`dead-air` と取り違えないこと(画面が変化しているかで弁別する)が最も重要。
演出側では同じ実演区間にzoomが集中する一方、フックとおわりには原則として
演出を置かない。`secret-exposure` のblurは区間に関係なく常に優先する。
