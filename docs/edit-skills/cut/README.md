...
`docs/edit-skills/cut/recipes/<id>.md`。id は kebab-case・`^[a-z][a-z0-9-]*$`。
...
3. **反例のうち「実際に起きた失敗」には出典を書く。** `rules.md` の3件
   ...
   `~/Movies/framewright/2026-07-12` の実データで確認できる「AI が cut と判断した
   候補を人間が GUI で keep に戻した」痕跡は、それぞれ対応する分類の反例節に
   **元秒つき**で書く

## craft knowledge の扱い(P6)

実データ・`rules.md` 由来ではない、一般に成文化された編集技法(craft
knowledge: L カット・アクションつなぎ・間の設計・言い直しの収束・フィラー
除去等)を `## 判定シグナル` へ書き足すときの規約。設計は
[`../plans/2026-07-21-cut-knowledge-p6-design.md`](../../plans/2026-07-21-cut-knowledge-p6-design.md)。

### 接地行の許可語彙(5種)

front-matter の `> 接地:` 行は、次の5種のトークンだけで構成する(機械検査
T-b が語彙の閉包を固定する)。

| トークン | 意味 |
|---|---|
| `実データ(…)` | 承認済み収録の `cutplan.json` から確認できる件数・出所 |
| `rules.md` | チャンネル `rules.md` に記録された失敗・型からの接地 |
| `想定` | 実データが無い合成例(G4)。`想定(…)` で由来を添えてもよい |
| `craft` | 収録を待たずに書ける、成文化された編集技法(本節の対象) |
| `観測: …` | 実データを確認したが該当が0件だった、等の記録 |

複数のトークンは ` + ` または ` · ` で連結する
(例: `実データ(2026-07-12, 23件) + craft`)。

### `craft` ラベルは `決定的`/`補助` と直交する第2軸

判定シグナルの箇条書きは `**決定的**`(1本で分類を決めてよい)/
`**補助**`(単独では使わない)の2値ラベルで始める。craft 由来のシグナルは
これに `(craft)` を併記する(例: `**補助(craft)**: …`)。実データ由来の
シグナルと視覚的に区別できることが、次の「実データ優先の規則」の前提になる。

### 実データ優先の規則(craft と実データが矛盾したときの規則)

**実データが常に勝つ。ただし craft は削らず「降格 + 観測併記」で残す**
(矛盾の多くは「craft が誤り」ではなく「この収録・このチャンネルでは出現
しなかった」という標本の薄さであり、消すと収録が増えたときに復元できない)。

| 対象 | 規則 |
|---|---|
| `## worked example` の実秒・実テキスト | **不可侵**。craft で書き換えない・補わない |
| `## 一行定義` / `## 既定の処置` | 実データが正。craft は上書きしない |
| `## 判定シグナル` | craft 由来は `**補助(craft)**` で入る。実データと矛盾する craft シグナルを `決定的` へ昇格させない(craft の `決定的` が許されるのは、実データが沈黙している=反証も追認もない場合のみ) |
| 観測との差 | front-matter の接地行へ `観測: …` として書く。節は増やさない |
| 発火0の分類 | 接地行に `観測: …` を書く。加えて該当 pattern の `recipes` から**落とす** |

### 出典の書き方(3条)

1. **技法名は名指ししてよい**(「L カット」「アクションつなぎ」「間の設計」)。
   一般に流通している呼称であり、これ自体は誰の著作物でもない
2. **原理は自分の言葉で書く。逐語引用をしない。書名・著者名・ページ番号を
   書かない**(著作権)。「◯◯という技法が知られている。その原理は〈自分の
   説明〉」の形にする
3. **craft 由来のシグナルには `craft` ラベルを付ける**(前述の第2軸)

**recipe の節構成は変えない。`## 出典` 節は新設しない**——出典は
front-matter の接地行(既存の機構)で表現する。
...
- **`rules.md`**: 「その状況でこのチャンネルはどうするか」という**強度**を
  上書きする、人間が育てる channel 固有資産(例: `~/Movies/framewright/rules.md`
...
`docs/edit-skills/cut/examples/*.md` は、実収録1本ぶんの判断を抜粋した記録
