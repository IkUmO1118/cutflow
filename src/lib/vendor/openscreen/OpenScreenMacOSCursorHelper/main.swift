// Vendored from OpenScreen (MIT). See ../PROVENANCE.md for origin + license.
//
...
// Resolves the CGDisplayBounds (Quartz global space: top-left origin, Y
// increases downward) to normalize the cursor against. Falls back to the
// main display when displayId is absent or no longer valid (e.g. unplugged).
func resolveTargetDisplayBounds(displayId: UInt32?) -> CGRect {
	if let displayId {
...
// accessibilityPointForMouse() already converts NSEvent.mouseLocation (Cocoa:
// bottom-left origin, Y up) into the global Quartz point (top-left origin, Y
// down) via a single flip anchored on the primary display's Cocoa height —
// the same axis CGDisplayBounds is expressed against, for every display, not
// just the primary one. So normalizing against `bounds` here must NOT flip Y
// again (that would double-flip and mirror cy top/bottom — the "混同の罠"
// this vendoring's design doc calls out).
// D4(対象ディスプレイの自動一致)の第3段(テレメトリ推論)は、録画開始前に
// 対象ディスプレイを決められなかったときのフォールバックで、どのディスプレイに
// 対して正規化すべきかを録画後まで判定できない。そのため ax/ay(グローバル
// Quartz 座標そのもの・正規化前)も併せて返し、Node 側(cursorGeom.ts)が
// 停止後にどのディスプレイの bounds が最も多くのサンプルを含むか判定して
// 事後的に再正規化できるようにする(cx/cy は spawn 時に解決済みの
// displayId(既定 primary)に対する正規化のまま=段1/2 の通常経路は不変)
func normalizedCursorPosition(
	in bounds: CGRect
) -> (cx: Double, cy: Double, inBounds: Bool, ax: Double, ay: Double) {
	let point = accessibilityPointForMouse()
	guard bounds.width > 0, bounds.height > 0 else {
		return (0, 0, false, Double(point.x), Double(point.y))
	}
	let cx = Double((point.x - bounds.minX) / bounds.width)
	let cy = Double((point.y - bounds.minY) / bounds.height)
	let inBounds = cx >= 0 && cx <= 1 && cy >= 0 && cy <= 1
	return (cx, cy, inBounds, Double(point.x), Double(point.y))
}
	...
	let decoded = try? JSONDecoder().decode(CursorHelperRequest.self, from: data)
{
	...
	request = CursorHelperRequest(sampleIntervalMs: nil, displayId: nil)
}

let intervalMs = max(8, request.sampleIntervalMs ?? 33)
let targetDisplayBounds = resolveTargetDisplayBounds(displayId: request.displayId)
...
while true {
	autoreleasepool {
		...
		let position = normalizedCursorPosition(in: targetDisplayBounds)
