...
// Source: src/components/video-editor/videoPlayback/zoomRegionUtils.ts (verbatim; imports rewritten to local .ts paths).
// Do not edit to "improve" — this is a faithful port. See ./PROVENANCE.md.
import type { CursorTelemetryPoint, Rotation3D, ZoomFocus, ZoomRegion } from "./types.ts";
import { DEFAULT_ROTATION_3D, getRotation3D, getZoomScale, lerpRotation3D } from "./types.ts";
import { TRANSITION_WINDOW_MS, ZOOM_IN_TRANSITION_WINDOW_MS } from "./constants.ts";
import { interpolateCursorAt } from "./cursorFollowUtils.ts";
import { clampFocusToScale } from "./focusUtils.ts";
import { clamp01, cubicBezier, easeOutScreenStudio } from "./mathUtils.ts";
