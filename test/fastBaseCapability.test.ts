...
test("resolveFastBaseCapability: camera無しplain designはscreen assetだけでactivateする", () => {
  ...
  delete props.cameraRegion;
  const result = resolveFastBaseCapability({ props, composite: false });
  assert.deepEqual(result, { ok: true, mode: "design", design: screenAssets });
});

test("resolveFastBaseCapability: camera無しdesignのscreen asset不足は理由付きで拒否する", () => {
  const { camera: _camera, ...screenOnly } = DESIGN;
  const props = propsWith({ ...screenOnly, assets: { key: ASSETS.key, backdropFile: ASSETS.backdropFile, screenMaskFile: "" } });
  delete props.cameraRegion;
