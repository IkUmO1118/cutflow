...
import { CUT_REASON_IDS, REASON_ID_LABEL, REASON_ID_DISCRIMINATOR } from "../src/lib/reasonIds.ts";
...
/* ------------------------------------------------------------------ */
/* T-n(P6-T7): 弁別行(REASON_ID_DISCRIMINATOR)の閉包・双方向参照・予算上限   */
/* ------------------------------------------------------------------ */

test("T-n(1): REASON_ID_DISCRIMINATOR の全ペア id が CUT_REASON_IDS に閉じる", () => {
  for (const { a, b } of REASON_ID_DISCRIMINATOR) {
    assert.ok((CUT_REASON_IDS as readonly string[]).includes(a), `弁別ペアの id "${a}" が CUT_REASON_IDS にありません`);
    assert.ok((CUT_REASON_IDS as readonly string[]).includes(b), `弁別ペアの id "${b}" が CUT_REASON_IDS にありません`);
  }
});

test("T-n(2): 各弁別ペアが対応する recipe の「紛らわしい隣」節で双方向に参照されている", () => {
  for (const { a, b } of REASON_ID_DISCRIMINATOR) {
    const aTokens = backtickedIdLikeTokens(relatedBlock(recipeSource(a)));
    assert.ok(aTokens.includes(b), `${a}.md の「紛らわしい隣」節が弁別ペアの相手 "${b}" を参照していません`);
    const bTokens = backtickedIdLikeTokens(relatedBlock(recipeSource(b)));
    assert.ok(bTokens.includes(a), `${b}.md の「紛らわしい隣」節が弁別ペアの相手 "${a}" を参照していません`);
  }
});

test("T-n(3): renderReasonIdsBlock(true, \"general\") は1800文字以下(§7 I9 の予算上限)", () => {
  const block = renderReasonIdsBlock(true, "general");
  assert.ok(block.length <= 1800, `general の注入ブロックが1800文字を超えています(実際 ${block.length}文字)`);
});
