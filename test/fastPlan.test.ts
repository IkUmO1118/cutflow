...
test("P-8: またぎ降格: zoom と部分的に重なる静止画 overlay は overlay 区間ごと SLOW へ降格", () => {
  const props = mkProps({
    durationSec: 30,
    zooms: [{ start: 10, end: 12, rect: { x: 0, y: 0, w: 100, h: 100 }, easeSec: 0 }],
...
test("P-9: 不動点2周: min-FAST 吸収が新たなまたぎを生み、2回目の反復で収束する", () => {
  const props = mkProps({
    durationSec: 30,
    zooms: [
      { start: 0, end: 1, rect: { x: 0, y: 0, w: 100, h: 100 }, easeSec: 0 }, // frame[0,30)
      { start: 10, end: 11, rect: { x: 0, y: 0, w: 100, h: 100 }, easeSec: 0 }, // frame[300,330)
    ],
...
test("P-10: 完全に SLOW の内側にある静止画 overlay は降格しない(spans 不変)", () => {
  const withOverlay = mkProps({
    durationSec: 30,
    zooms: [{ start: 5, end: 15, rect: { x: 0, y: 0, w: 100, h: 100 }, easeSec: 0 }],
