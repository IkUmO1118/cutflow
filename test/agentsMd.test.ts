// AGENTS.md(リポジトリ直下・英語・Claude 非依存の機械可読契約)の列挙が、
...
test("AGENTS.md: 編集ファイル表が8編集ファイルを過不足なく含む", () => {
  for (const f of EDITABLE_FILE_NAMES) {
    assert.ok(AGENTS_MD.includes(`\`${f}\``), `${f} が AGENTS.md に見つかりません`);
  }
});

test("AGENTS.md: deny節がGENERATED_FILES + APPROVAL_FILEを過不足なく含む", () => {
  for (const f of GENERATED_FILES) {
    assert.ok(AGENTS_MD.includes(f), `${f}(GENERATED_FILES)が AGENTS.md に見つかりません`);
  }
  assert.ok(AGENTS_MD.includes(APPROVAL_FILE), `${APPROVAL_FILE}(APPROVAL_FILE)が AGENTS.md に見つかりません`);
});

test("AGENTS.md: コマンド表がCLI登録の全コマンド名を過不足なく含む", () => {
  const names = extractCliCommandNames(CLI_TS);
  assert.ok(names.length > 0);
  for (const name of names) {
    assert.ok(AGENTS_MD.includes(`\`${name} <dir>\``), `${name} が AGENTS.md のコマンド表に見つかりません`);
  }
});

test("AGENTS.md: id接頭辞節がID_PREFIXの全値を過不足なく含む", () => {
  const prefixes = Object.values(ID_PREFIX);
  assert.ok(prefixes.length > 0);
  for (const prefix of prefixes) {
    assert.ok(AGENTS_MD.includes(`\`${prefix}\``), `接頭辞 ${prefix} が AGENTS.md に見つかりません`);
  }
});

test("AGENTS.md: Claude固有語(claude -p / Claude Code)を持ち込まない", () => {
