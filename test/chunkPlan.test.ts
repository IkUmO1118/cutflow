...
test("chunkVideoKey: overlay keyframes の変更は重なるチャンクだけを変える", () => {
  const withOverlay: RenderProps = {
    ...PROPS,
    overlays: [{
      start: 1,
      ...
      keyframes: [
        { at: 1, easing: "linear", values: { x: 0 } },
        { at: 2, easing: "linear", values: { x: 100 } },
      ],
    }],
  };
  const before = keysOf(withOverlay);
  const after = keysOf({
    ...withOverlay,
    overlays: [{ ...withOverlay.overlays[0], keyframes: [{ at: 1, easing: "linear", values: { x: 50 } }] }],
  });
  assert.notEqual(before.chunk0, after.chunk0);
  assert.equal(before.chunk1, after.chunk1);
});

test("chunkVideoKey: annotations は重なるチャンクだけキーに入る", () => {
  const withAnnotation: RenderProps = {
    ...PROPS,
    annotations: [{
      start: 1,
      end: 2,
      ...
      widthPx: 6,
      radiusPx: 8,
      keyframes: [{ at: 1, easing: "linear", values: { x: 10 } }],
    }],
  };
  const before = keysOf(PROPS);
  const after = keysOf(withAnnotation);
  assert.notEqual(before.chunk0, after.chunk0);
  assert.equal(before.chunk1, after.chunk1);
  assert.equal(before.audio, after.audio);
});
