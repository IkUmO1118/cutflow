...
[`assets/fonts/NotoSansJP.woff2`](../../../assets/fonts/NotoSansJP.woff2).
...
- Glyph text: `FrameWright WOFF2 埋め込みフォント 0123456789`

The file was produced with an external, temporary fonttools installation. FrameWright
does not depend on or bundle fonttools:

```sh
PYTHONPATH=/private/tmp/framewright-x1-fonttools \
/private/tmp/framewright-x1-fonttools/bin/pyftsubset \
  assets/fonts/NotoSansJP.woff2 \
  --output-file=test/fixtures/hyperframe-fonts/NotoSansJP-X1-subset.woff2 \
  --flavor=woff2 \
  '--text=FrameWright WOFF2 埋め込みフォント 0123456789' \
  '--layout-features=*' \
  --no-hinting
```
...
`docs/hyperframes-skills/examples/framewright--embedded-woff2-font.html` intentionally
