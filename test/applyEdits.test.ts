...
test("P2-9: compileOps: set で cutplan segment の reasonId を書ける(field パスは既存の許可キー集合を制限しない)", () => {
  const docs = baseDocs();
  const ops: EditOp[] = [{ op: "set", target: "@seg_a1a1a1", field: "reasonId", value: "demo-wait" }];
  const { body, errors } = compileOps(docs, ops);
  assert.deepEqual(errors, []);
  assert.equal(body.cutplan!.segments[0].reasonId, "demo-wait");
  // 他フィールドは不変
  assert.equal(body.cutplan!.segments[0].reason, "本編");
  assert.equal(body.cutplan!.segments[0].action, "keep");
});

test("P2-9: compileOps: set で reasonId を別分類へ書き換えられる(cut系→boundary系)", () => {
  const docs = baseDocs();
  const ops: EditOp[] = [{ op: "set", target: "@seg_b2b2b2", field: "reasonId", value: "tail-clip" }];
  const { body, errors } = compileOps(docs, ops);
  assert.deepEqual(errors, []);
  assert.equal(body.cutplan!.segments[1].reasonId, "tail-clip");
});

test("P2-9: compileOps: remove(@id 単位)は reasonId 付き segment でも従来どおり所属配列から抜く", () => {
  const docs = baseDocs();
  docs.cutplan!.segments[1] = { ...docs.cutplan!.segments[1], reasonId: "restatement" };
...
test("P2-9: compileOps: reasonId を set しても approved は依然として set の field にできない(承認境界の再確認)", () => {
  const docs = baseDocs();
  const ops: EditOp[] = [
    { op: "set", target: "@seg_a1a1a1", field: "reasonId", value: "demo-wait" },
    { op: "set", target: "cutplan", field: "approved", value: true },
  ];
  const { errors } = compileOps(docs, ops);
  assert.ok(errors.some((e) => e.message.includes("承認")), "approved を set しようとするとエラーになるべき");
});
...
test("P2-9: applyEdits: set で cutplan segment の reasonId をディスクへ書ける(検査付きアトミック書込み)", () => {
  withTmpProject((dir) => {
    ...
    assert.deepEqual(result.written, ["cutplan.json"]);
    const cutplan = JSON.parse(readRaw(dir, "cutplan.json"));
    assert.equal(cutplan.segments[0].reasonId, "demo-wait");
  });
});
