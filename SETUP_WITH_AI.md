...
FrameWright の実行環境を自動でセットアップさせる**ための指示書です。

## 使い方(人間向け)

クローン後、エージェントにこう言うだけ:

> **「`SETUP_WITH_AI.md` を読んで、その手順どおりに FrameWright のセットアップをして。
...
あなたは FrameWright の環境をセットアップするエージェントです。以下を **順に・冪等に**
...
- `v23.6.0` 未満 なら **停止**して案内: FrameWright は TypeScript を type-stripping で
