...
| `shorts.json` | ショート動画(縦)の元データ。`shorts[]` の各要素が `{name, profile?, approved, ranges[], captionTracks?}`。`name` は出力ファイル名(`shorts/<name>.mp4`)。`ranges`(元収録の秒)が本編 `cutplan.json` とは独立のこのショート専用 keep 集合(本編でカットした素材も含められる)。`profile` は `vertical`(既定)/ `vertical-cover` / `default` の組み込みレイアウト。`captionTracks` は `overlays.json` と同型の縦用テロップ位置/スタイル上書き。`approved` はこのショート単体の承認(**AI は自分で true にしない**) |
