...
| `transcript.json` | テロップの文言・表示時間。`track`(トラック番号)、`pos`(`{x,y}` 出力px)、`style`(`{fontSizePx, color, outlineColor, fontFamily, fontWeight, background}`。`outlineColor: "none"` で縁なし、`background` は座布団 `{color, paddingPx?, radiusPx?}`)は省略可で個別上書き。`words`(語/トークン単位のタイミング。whisper `-ojf` 由来、`config.yaml` の `whisper.wordTimestamps: true` のときだけ付く。省略可・**カラオケアニメ等の描画専用の補助データで text/start/end が常に正**。人間が text を手編集しても words は追随しない) |
