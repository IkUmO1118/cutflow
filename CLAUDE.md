...
- 中間生成物は編集しない(再実行で上書きされる): `manifest.json` /
  ...
  透過 PNG(内容+解決済みスタイル+位置+出力解像度のハッシュがキー)。差分
  更新型でディレクトリごと削除すれば常にフル再生成に戻る) /
