...
- 編集 = 収録フォルダ内の `cutplan.json` / `transcript.json` / `overlays.json` /
  `chapters.json` / `meta.json` の編集。動画ファイル自体は触らない
...
| `overlays.json` | 演出。`overlays`(素材の全画面表示)/ `inserts`(インサート編集)/ `wipeFull`(ワイプ全画面)/ `layerOrder`(重なり順)/ `captionTracks`(テロップトラックの標準位置・スタイル)/ `hideCaption` |
