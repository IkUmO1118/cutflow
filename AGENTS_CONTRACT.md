...
- Fixed-name generated files: `manifest.json` (**not** deleted by `clean` —
  ...
  work), `preview.mp4`, `preview-cut.mp4` (the continuous baked cut video for the
  editor preview; no longer referenced by the client, kept classified so
  `clean` can reclaim leftovers), `proxy.mp4`, `proxy.key.json`,
