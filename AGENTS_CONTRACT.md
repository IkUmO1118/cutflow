...
| `editor <dir>` | Launch the GUI editor, creating `<dir>` when it does not exist. An empty folder or ambiguous base-media candidates open normally for explicit GUI selection; `--canvas <preset>` is accepted only for first bootstrap, and an existing different canvas is rejected |
| `mcp <dir>` | Launch a Model Context Protocol server over stdio, bound to this one recording folder (§11) |
| `run <dir>` | First-time bulk pipeline: ingest → transcribe → detect → plan → id-stamp, then non-destructively invokes `autozoom`'s logic if fresh (§9: do not re-run casually). Accepts the same creation-time `--canvas <preset>` as `ingest` |
